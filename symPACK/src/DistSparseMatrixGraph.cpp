...
namespace symPACK{
  ...
  void DistSparseMatrixGraph::SetExpanded(int aExpanded) {
    ...
    if(expanded != aExpanded){
      if(aExpanded){
        ...
        //std::cout << "ExpandSymmetric" << std::endl;
        ExpandSymmetric();
      }
      else{
        //restrict to symmetric storage
        //std::cout << "ToSymmetric" << std::endl;
  ...
  void DistSparseMatrixGraph::ExpandSymmetric(){
    ...
    if(!expanded){
      ...
      //std::cout << "stage 2: expanded? " << expanded << std::endl;
    ...
    if (colptr.empty() || rowind.empty()) {
    std::cerr << "Error: colptr or rowind arrays are empty!" << std::endl;
    return;
}
...
// Start of the counting timer
SYMPACK_TIMER_START(DistMat_Expand_count);

try {
    // Initial Variables
    //std::cout << "Initial Variables:\n";
    //std::cout << "locColCnt: " << locColCnt << std::endl;
    //std::cout << "firstLocCol: " << firstLocCol << std::endl;
    //std::cout << "baseval: " << baseval << std::endl;
    //std::cout << "mpisize: " << mpisize << std::endl;

    // Array Sizes
    //std::cout << "colptr size: " << colptr.size() << std::endl;
    //std::cout << "rowind size: " << rowind.size() << std::endl;
    //std::cout << "vertexDist size: " << vertexDist.size() << std::endl;

    // Loop through local columns
    for(Idx locCol = 0; locCol < locColCnt; locCol++) {
        Idx col = firstLocCol + locCol;  // 0-based

        //std::cout << "Processing locCol: " << locCol << ", col: " << col << std::endl;

        Ptr colbeg = colptr[locCol] - baseval; // now 0-based
        Ptr colend = colptr[locCol+1] - baseval; // now 0-based
        //std::cout << "colbeg: " << colbeg << ", colend: " << colend << std::endl;
        
        for(Ptr rptr = colbeg; rptr < colend; rptr++) {
            Idx row = rowind[rptr] - baseval; // 0-based
            //std::cout << "rptr: " << rptr << ", row: " << row << std::endl;
            
            if(row > col) {
                // Determine the processor destination for the row
                Idx pdest = 0;
                int iterations = 0;

                while(pdest < mpisize && !(row >= vertexDist[pdest] - baseval && row < vertexDist[pdest+1] - baseval)) {
                    pdest++;
                    iterations++;
                    if(iterations > mpisize) {
                        std::cerr << "Error: Infinite loop detected.\n";
                        break;  // Break out to prevent infinite loop
                    }
                }

                // Safety check: Ensure pdest is valid
                if(pdest >= mpisize) {
                    std::cerr << "Error: Row index " << row << " does not belong to any processor range.\n";
                    throw std::out_of_range("Row index does not belong to any processor range.");
                }

                // Increment the count for the appropriate processor
                ssizes[pdest]++;
            }
        }
    }
