...
#pragma unroll
  ...
  template <typename colptr_t, typename rowind_t, typename T, typename int_t = int> 
    class symPACKMatrix2D: public symPACKMatrixMeta<T>{
      ...
      virtual void ExtractNonZeroElements(std::vector<int>& rowIndices, std::vector<int>& colIndices, std::vector<double>& nonZeroValues);
...
template <typename colptr_t, typename rowind_t, typename T, typename int_t>
void symPACKMatrix2D<colptr_t,rowind_t,T,int_t>::ExtractNonZeroElements(
    ...
    std::vector<double>& nonZeroValues) 
{
    for (Int I=1; I < this->Xsuper_.size(); I++) {
        Int src_first_col = this->Xsuper_[I-1];
        // Int src_last_col = this->Xsuper_[I]-1;  // Not used
            ...
            if (this->cells_.find(idx) != this->cells_.end()) {
                ...
                if (iOwner == this->iam) {
                    ...
                    for (auto &block : tgt_cell.blocks()) {
                        ...
                        for (Int i = 0; i < nRows; ++i) {
                            for (Int j = 0; j < tgt_cell.width(); ++j) {
                                T value = val[i*tgt_cell.width() + j];
                                if (value != 0) {  // Assuming '0' means zero in your data type
                                    rowIndices.push_back(row+i);
                                    colIndices.push_back(src_first_col+j);
...
template <typename colptr_t, typename rowind_t, typename T, typename int_t>
double symPACKMatrix2D<colptr_t,rowind_t,T,int_t>::ComputeLogDeterminant() 
{
    double logDetL = 0.0;
            ...
            if (this->cells_.find(idx) != this->cells_.end()) {
                ...
                if (iOwner == this->iam) {
                    ...
                    for (auto &block : tgt_cell.blocks()) {
                        ...
                        if (I == J) {  // Only when I == J we're on the diagonal block
                            for (Int i = 0; i < nRows; ++i) {
                                T value = val[i*tgt_cell.width() + i]; // Diagonal entry
                                logDetL += std::log(value);
                            }
                        }
                    }
                }
